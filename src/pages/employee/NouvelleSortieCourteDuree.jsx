...
export default function NouvelleSortieCourteDuree() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!dateSortie) {
      setFormError("Veuillez renseigner la date de sortie.");
      return;
    }
    ...
    if (!heureRegex.test(heureDebut)) {
      setFormError("L'heure de début doit être au format HH:MM.");
      return;
    }

    if (!heureRegex.test(heureFin)) {
      setFormError("L'heure de fin doit être au format HH:MM.");
      return;
    }
    ...
    try {
      ...
      setFormError(error || "Erreur lors de l'ajout de la demande de sortie.");
    } finally {
      setSubmitting(false);
    }
  };
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
              <label className="text-sm font-semibold text-slate-700 block mb-2">
                Date sortie <span className="text-red-500">*</span>
              </label>
              <input
                type="date"
                value={dateSortie}
            ...
            <div className="sm:col-span-2">
              ...
              <textarea
                ...
                placeholder="Expliquez la raison de votre sortie..."
                required
              />
            </div>
          </div>
